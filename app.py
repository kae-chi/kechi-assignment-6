...
def generate_plots(N, mu, sigma2, S):
    ...
    X = np.random.uniform(-5, 5, N)  # Generate N random values between 0 and 1
    Y = X + np.random.normal(mu, np.sqrt(sigma2), N)  # Normal additive error with specified mean and variance
